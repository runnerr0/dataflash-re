...
- ✅ **Bridge TX updated to the REAL 8-head broadcast** (2026-06-27): `dataflash_tx.sendRefresh` now emits `55 40 + <8-bit per fixture> + 00` (no ARM/START/FIRE), `fixtureCount` default 8, DMX mapped full-8-bit, patterns scaled 4-bit→8-bit. Builds clean (all envs). NOTE: the loopback "137/137 as `55 7F 88×8 F7`" above validated the *old* framing; `tools/dataflash_frame.py` still mirrors that legacy packet and is now out of sync with the bridge — re-loopback against the new broadcast when convenient. Not yet hardware-re-verified on the new framing.
