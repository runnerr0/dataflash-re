...
- ✅ **Bridge nibble-packing verified** via asymmetric capture (2026-06-25): fixture 0→`F0` (even=high nibble), fixture 1→`0F` (odd=low nibble) — matches `addr÷2`=byte index, `addr&1`=nibble. (This validated the OLD 256-head marker framing; superseded below.)
- ✅ **Bridge TX updated to the REAL 8-head broadcast + HARDWARE-RE-VERIFIED** (2026-06-27): `dataflash_tx.sendRefresh` emits `55 40 + <8-bit per fixture> + 00` (no ARM/START/FIRE), `fixtureCount` default 8, DMX mapped full-8-bit, patterns scaled 4-bit→8-bit. **Loopback re-verified on the wire: 139/139 frames decoded exactly as `55 40 FF×8 00`** (bridge all-on test, captured via USB-RS485 8E1, decoded with the updated `tools/dataflash_frame.py`). Builds clean (all envs).
...
5. **Audio-in MVP (env `esp32-s3-audio`):** dual-I2S (INMP441 mic / line-in ADC, SW-select) DSP task with a **biquad band selector** (Full/Bass/Mid/Treble) → three mappings: **Level** (band amplitude→intensity), **Pulse** (band onset→all-flash), **Advance** (onset→stage). Beat counting on the selected band (`g_aud.beats`). OSC `/df/audio/{enable,source,mode,band,gain,gate,pulse}`. Builds clean. TODO: verify I2S pins on real S3-ETH header (placeholders 8-13), line-in MCLK if the PCM card needs it, web-UI controls + level meter, FFT **Spectrum** (bands→heads) + **Tempo-lock** (phase 2), respect fixture cooldown duty cap.
