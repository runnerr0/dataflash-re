...
## Per-fixture byte semantics — RESOLVED empirically: 8-bit intensity (8-head)  [wire]
**Update 2026-06-27 — master-knob sweep settles the 8-head byte.** Running a sparse program (prog 26, one head lit) and sweeping the controller's master intensity knob, the lit head's byte decreases **monotonically as a single number**: FULL `0x80` (128) → ~2/3 `0x55` (85) → HALF `0x17` (23) → MIN `0x0F` (15). So the 8-head per-fixture byte is **a single ~8-bit intensity**, NOT the `[4-bit mode | 4-bit intensity]` derived below from the 256-head firmware. The nibble "patterns" were just a falling byte decomposing (high nibble = high bits, low nibble = remainder); programs cluster on discrete values only because each picks specific intensity LEVELS. First principles holds: 8 fixtures = enough bandwidth for full 8-bit each, vs the 256-head's 2×4-bit cram. The `[mode|intensity]` derivation below is **correct for the 256-head firmware** but **superseded for the 8-head** by this measurement. (Still open: whether strobe-*effect* programs encode flash-rate/mode somewhere; the **Flash/Fire button** is just `55 40` + `80×8` = all heads full, no markers.)
