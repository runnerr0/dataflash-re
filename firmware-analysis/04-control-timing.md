...
- **Fix applied** in `bridge/src/dataflash_rx.cpp`: sample data bits at 0.30 into each bit window (was 0.50) for ~0.70 bit of slack against late `t0`; 9th bit at its window center. A confirmatory re-capture should show `0x80`/`0xC0` drop toward zero.
